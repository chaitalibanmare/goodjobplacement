...
const AddCourse = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Notes PDFs
    // ✅ Notes validation (MANDATORY for new courses only)
    const validNotes = form.notes.filter(note => note !== null && typeof note !== 'string');

    if (!editData && validNotes.length === 0) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || data.details || "Failed to save course");
      }
      ...
      <div className="course-form-container">
        <form className="course-form" onSubmit={handleSubmit}>
          ...
          <div className="form-row">
            ...
            {editData && editData.image && (
              <div style={{ marginBottom: "10px" }}>
                <p style={{ margin: "0 0 8px 0", fontSize: "14px", fontWeight: "bold", color: "#475569" }}>Previously uploaded image:</p>
                <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
                  <img src={`http://localhost:5000/uploads/${editData.image}`} alt="course" style={{ width: "80px", height: "50px", objectFit: "cover", borderRadius: "8px", border: "1px solid #ddd" }} />
                  <a href={`http://localhost:5000/uploads/${editData.image}`} target="_blank" rel="noreferrer" style={{ fontSize: "14px", color: "#2563eb", textDecoration: "underline", fontWeight: "bold" }}>
                    View Image
                  </a>
                </div>
              </div>
          ...
          <div className="form-row">
            ...
            {editData && editData.notes && editData.notes.length > 0 && (
              <div style={{ marginBottom: "15px", padding: "10px", background: "#f8fafc", borderRadius: "8px", border: "1px solid #e2e8f0" }}>
                <p style={{ margin: "0 0 8px 0", fontSize: "14px", fontWeight: "bold", color: "#475569" }}>Previously uploaded notes:</p>
                <ul style={{ margin: 0, paddingLeft: "20px", fontSize: "14px" }}>
                  {editData.notes.map((note, index) => (
                    <li key={index} style={{ marginBottom: "6px" }}>
                      <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                        <span style={{ color: "#555" }}>{note.length > 25 ? note.substring(0, 25) + '...' : note}</span>
                        <a href={`http://localhost:5000/uploads/${note}`} target="_blank" rel="noreferrer" style={{ color: "#2563eb", textDecoration: "underline", fontWeight: "bold" }}>
                          View Notes
                        </a>
                      </div>
                    </li>

...
function Navbar({onOpenLogin, onOpenSettings, user, onLogout}){
  ...
  const getProfilePhotoUrl = () => {
    if(user && user.profile && user.profile.photo){
      ...
      return base + user.profile.photo
    }
    return null
  }
          ...
          {user ? (
            <>
              <div style={{display:'flex', alignItems:'center', gap:12}}>
                <span style={{marginRight:0}}>Hi, {user.name}</span>
                {getProfilePhotoUrl() && (
                  <img
                    src={getProfilePhotoUrl()}
                    alt={user.name}
                    onClick={() => navigate('/profile')}
                    style={{width:40, height:40, borderRadius:'50%', objectFit:'cover', border:'2px solid #4CAF50', cursor:'pointer'}}
                    title="View profile"
                  />
                )}
              </div>
    ...
    />
      ...
      <main>
        <Routes>
          ...
          <Route path="/courses" element={user ? <UserCourses /> : <Courses />} />
          <Route path="/features" element={<Features />} />
          <Route path="/placement-activity" element={<PlacementActivity />} />
          <Route
            path="/community"
            element={
              localStorage.getItem("gjp_user")
                ? <UserCommunity />
                : <Community />
            }
          />
          ...
          <Route path="/admin/*" element={<AdminLayout />}>
            ...
            <Route path="activity" element={<PlacementActivity />} />
            <Route path="userProfile/:id" element={<UserProfile />} />
            <Route path="staff" element={<StaffList />} />
            <Route path="community" element={<ManageCommunities />} />
          </Route>
